...
rooms = [
    ...
    "Hotel Lower Lobby",
    "Sponsors Suite",
    "H. Honors Lounge", "H. Honors Lounge 2"
]


def plot_from_reader(
    reader, worksheet, time_lookup, room_lookup, vert_offset, panel_format
):
    for row in reader:
        title = row["title"]
        room = row["room"]
        ...
        if room in room_lookup:
            ...
            if length == 30:
                worksheet.write(
                    v_off + room_lookup[room], time_lookup[time], title, panel_format
                )
            else:
                add_cols = int(int(length) / 30) - 1
                print(f"{title} is {add_cols}+1 wide")
                worksheet.merge_range(
                    v_off + room_lookup[room],
                    time_lookup[time],
                    v_off + room_lookup[room],
                    time_lookup[time] + add_cols,
                    title,
                    panel_format,
                )
